...
export default function Slack() {
  ...
  const sendNotification = async () => {
    if (sendUrl && notification) {
      const data = {
        send_url: sendUrl,
        notification
      }
      axios.post('http://localhost:8000/api/v1/send', {
        platform: 'slack',
        method: 'sendMsg',
        custom: data
      })
    ...
    <div>
      ...
      <section className='mt-5'>
        <div className='grid gap-4 p-5 rounded-lg border-4 border-dashed border-gray-200'>
          <Input
            labelText='Slack API URL'
            name='sendUrl'
            type='text'
            placeholder='Slack send api url'
            setValue={setSendUrl}
            value={sendUrl}
          />
          <TextArea
      ...
      <section className='mt-20 p-4'>
        <Example platform='Slack' args={[
          { title: 'Send API URL', example: 'https://hooks.slack.com/services/******/*******/******' },
